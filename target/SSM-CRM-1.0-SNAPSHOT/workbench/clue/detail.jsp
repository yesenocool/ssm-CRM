...
<script type="text/javascript">
	...
	function  bundBtn() {
		...
		if($xz.length!=0){
			if(confirm("是否关联")){
				...
				$.ajax({
					url:"${pageContext.request.contextPath}/active/bundActivity",
					data:parm,
					dataType:"json",
					type:"post",
	...
	// 动态查询没有关联的市场活动，展示出来
	function unbundActive() {
		$.ajax({
			url:"${pageContext.request.contextPath}/active/getActivityListByNameAndNotByClueId",
			data:{
				"name":$.trim($("#aname1").val()),
				"clueId":"${c.id}"
			},
			dataType:"json",
			type:"get",
	...
	function showActivityList() {
		$.ajax({
			url:"${pageContext.request.contextPath}/clue/ActivityListByClueId",
			data:{
				"id":"${c.id}"
			},
			dataType:"json",
