...
<script type="text/javascript">
	$(function(){
		...
		//通过关键字模糊搜索市场活动
		$("#Btn01").click(searchActivityByName);
		//获取选中的市场活动，并放入文本框中。
		$("#submitActivityBtn").click(getSelectedActivity);
		//绑定转换事件
	...
	function convertBtn() {
		/*
		* 这里转换完成后会进行页面跳转，而转换的操作更适合用传统请求
		*请求结束后返回线索列表页面
		* 根据“为顾客创建交易 是否被选中来决定是否创建交易
		*
		* */
		if($("#isCreateTransaction").prop("checked")){
			...
			* */
			...
			/*
			* 如果没有被选中则传递一个clueid就可以了
			* */
			window.location.href="${pageContext.request.contextPath}/clue/convert.do?clueId=${param.id}";
		}
	...
	function searchActivityByName() {
		...
		$.ajax({
			url:"${pageContext.request.contextPath}/active/searchActivityByName",
			data:{
				"name":name
			},
			dataType:"json",
...
<body>
	...
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
		...
		<form id="tranForm" action="${pageContext.request.contextPath}/clue/convert.do" method="post">
